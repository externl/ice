// Minimal Bluetooth RFCOMM echo helper used to probe whether two Android emulators can do RFCOMM
// (and bonding, for secure sockets) over Netsim/Rootcanal on CI. Not part of Ice; spike-only.
//
...
public class MainActivity extends Activity {
    ...
    // Best-effort auto-accept of incoming pairing requests. setPairingConfirmation/setPin need the
    // privileged BLUETOOTH_PRIVILEGED permission; if we don't have it this logs the SecurityException,
    // which is itself the signal we want about whether headless bonding is possible.
    ...
    void run(String mode, String peer, String uuidStr, boolean secure, boolean bond) {
        try {
            ...
            Log.i(TAG, "adapter address=" + adapter.getAddress() + " enabled=true");
            ...
            if ("server".equals(mode)) {
                ...
                Log.i(TAG, "accepted from " + s.getRemoteDevice().getAddress());
